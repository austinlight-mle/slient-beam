...
PORT = 8082  # v2 server listens on a different port
...
@app.route("/v2/<client_id>/<monitor_num>/upload", methods=["POST"])  # v2 endpoint path
def upload(client_id: str, monitor_num: str):
    ...
    with _clients_lock:
        ...
        if not client:
            ...
            _log(f"[v2] New client discovered: {client_id}")
        ...
        if not mon_entry:
            ...
            just_created = True
            _log(f"[v2] {client_id} monitor {monitor_num} discovered")
        ...
        if just_created:
            try:
                ...
                age = "0s"
            _log(f"[v2] {client_id} monitor {monitor_num} registered: last_seen={age}, uploads={mon_entry['uploads']}")

    _log(f"[v2] {client_id} m{monitor_num}: received {saved_count} file(s)")
    return "OK", 200


def _run_flask():
    _log(f"Server v2 starting on 0.0.0.0:{PORT}")
...
def _launch_gui():
    root = tk.Tk()
    root.title("SilentBeam Server v2")
            ...
            for mk, me in sorted(monitors.items(), key=lambda kv: int(kv[0]) if str(kv[0]).isdigit() else str(kv[0])):
                ...
                with _clients_lock:
                    ...
                    if current != desired:
                        ...
                        _log(f"[v2] {cid} monitor {mk} status -> {desired.upper()}")
